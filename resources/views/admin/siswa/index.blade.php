@extends('layouts.app', ['title' => 'Siswa - Admin'])
                ...
                <table class="min-w-full table-auto">
                    <thead class="justify-between">
                        <tr class="bg-gray-600 w-full">
                            <th class="px-16 py-2">
                                <span class="text-white">NAMA SISWA</span>
                            </th>
                            <th class="px-16 py-2 text-left">
                                <span class="text-white">TANGGAL LAHIR</span>
                            </th>
                            <th class="px-16 py-2">
                                <span class="text-white">ALAMAT SISWA</span>
                            </th>
                            <th class="px-16 py-2 text-left">
                    ...
                    @forelse($siswas as $siswa)
                        <tr class="border bg-white">
                            ...
                            <td class="px-16 py-2">
                            {{ $siswa->ruang->nama_ruang }}
                            </td>
            
                            <!--action untuk tombol hapus dan edit belum ditambahkan -->
                            <td class="px-10 py-2 text-center">
                                <a href="{{ route('admin.siswa.edit', $siswa->id) }}" class="bg-indigo-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">EDIT</a>
                                <button onClick="destroy(this.id)" id="{{ $siswa->id }}" class="bg-red-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">HAPUS</button>
                            </td>
                        </tr>
                        @empty
