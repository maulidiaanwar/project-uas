@extends('layouts.app', ['title' => 'Guru - Admin'])
    ...
    <div class="container mx-auto px-6 py-8">
        ...
        <div class="flex items-center">
            <button class="text-white focus:outline-none bg-gray-600 px-4 py-2 shadow-sm rounded-md">
                <a href="{{ route('admin.guru.create') }}">TAMBAH</a>
            </button>
                ...
                <table class="min-w-full table-auto">
                    ...
                    @forelse($gurus as $guru)
                        <tr class="border bg-white">
                            ...
                            <td class="px-16 py-2">
                            {{ strip_tags($guru->alamat) }}
                            </td>
            
                            <!--action untuk tombol hapus dan edit belum ditambahkan -->
                            <td class="px-10 py-2 text-center">
                                <a href="{{ route('admin.guru.edit', $guru->id) }}" class="bg-indigo-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">EDIT</a>
                                <button onClick="destroy(this.id)" id="{{ $guru->id }}" class="bg-red-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">HAPUS</button>
                            </td>
                        </tr>
                        @empty
